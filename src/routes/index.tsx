    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <main className="relative mx-auto flex max-w-5xl flex-col items-center px-6 pb-20 pt-12 text-center md:pt-20">
        <span className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card/60 px-4 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur">
          ...
          A calm strategy game about India's data protection law
        </span>
        ...
        <p className="mt-6 max-w-xl text-balance text-base text-muted-foreground md:text-lg">
          A Monopoly-style multiplayer learning game that turns the Digital Personal Data
          Protection Act 2023 into a quiet boardroom you actually want to play in.
        </p>
        ...
        <div className="mt-10 flex flex-col items-center gap-3 sm:flex-row">
          <Button
            ...
            onClick={() => navigate({ to: user ? "/lobby" : "/auth", search: user ? undefined : { mode: "signup" } })}
          >
        ...
        <div className="mt-20 grid w-full grid-cols-1 gap-4 sm:grid-cols-3">
          {[
            { icon: Users, title: "2–4 players", body: "Solo vs. Computer or invite friends with a room link." },
            { icon: Shield, title: "9 principles", body: "Each tile teaches one part of the Act through MCQs." },
            { icon: Dices, title: "20–45 min", body: "Roll, answer, build compliance — last one standing wins." },
          ].map(({ icon: Icon, title, body }) => (
            <div key={title} className="rounded-2xl border border-border bg-card/70 p-5 text-left shadow-soft backdrop-blur">
